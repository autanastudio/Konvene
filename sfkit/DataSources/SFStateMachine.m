...
static NSString * const SFStateNil = @"Nil";

@implementation SFStateMachine {
    OSSpinLock _lock;
}
...
- (id)target
{
    id<SFStateMachineDelegate> delegate = self.delegate;
    if (delegate)
        return delegate;
    return self;
}
...
- (void)_performTransitionFromState:(NSString *)fromState toState:(NSString *)toState
{
    ...
    NSString *fromStateNotNil = fromState ? fromState : SFStateNil;
